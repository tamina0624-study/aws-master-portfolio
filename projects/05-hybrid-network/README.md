...
> 「オンプレミスの基幹システムをすぐにクラウドへ移せない。
> でも AWS の新しいサービスも使いたい。両方を安全につなぐ方法はないの？」

> 「VPN が 1 本しかないから、回線障害が起きたら全拠点が AWS にアクセスできなくなる。」

既存のオンプレミス環境と AWS を安全に接続するのが **ハイブリッドネットワーク** です。
**Site-to-Site VPN** を使えば、インターネット経由で暗号化されたトンネルを張り、オンプレと AWS を仮想的に同一ネットワークのように扱えます。
さらに **BGP（動的ルーティング）** を組み合わせると、2 本のトンネルで冗長化でき、片方が落ちても自動的に経路を切り替えられます。
物理ルーターがなくても EC2 + strongSwan で同等の環境を再現できるため、AWS 上で完結したシミュレーション検証も可能です。

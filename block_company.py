...
keywords = load_keywords_section("projects/00-Secret/keywords.txt", "company")
            ...
            if line.startswith('+++ b/'):
                ...
                for pattern in patterns:
                    if pattern.search(content) and key not in already_reported:
                        f.write(f"ERROR: Company detected in {current_file} at line {current_line}: {content.strip()}\n")
                        found = True

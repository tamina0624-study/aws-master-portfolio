...
> 「オンプレとAWSをVPNでつないだはいいけど、
> 外部からの攻撃を検知してもブロックするまでに時間がかかる。
> VPN 越しに内部ネットワークまで侵入されたらどうしよう…」

> 「WAF は設置したけど、拒否リストを手動で更新している。
> リアルタイムの攻撃に追いつけていない。」

ハイブリッドクラウド環境では、オンプレとクラウドの両方を守る必要があります。
**Site-to-Site VPN** で安全に接続しながら、**GuardDuty** が異常を検知したら即座に **Lambda** が **WAF** の拒否リストへ自動登録する、自律防御のループを構築することで「気づいたら止まっている」状態を実現できます。
WAF・NACL・Security Group を役割に応じて使い分け、多層でネットワークを守る設計が、エンタープライズでは求められます。
